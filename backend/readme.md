# 백엔드
### 공통 : navbar 로고를 누르면 index로 이동, 오른쪽 5개 바로가기 링크
----------------------------------------------------------------------
## index
### 제일 먼저 보이는 페이지
![Image](https://github.com/user-attachments/assets/0042b09c-e89e-4060-83f4-9467128efa98)

## search
### index, search에서 제목(작가x)을 검색하면 사람들이 올린 책들을 한눈에 보여줌. 자세히보기(bookdetail.jsp)에서 판매가, 설명 확인 가능
![Image](https://github.com/user-attachments/assets/0c269b7b-7657-4f2b-a7e7-9413f25016d8)

## signup
### 회원가입에 정보를 적고 회원가입
![Image](https://github.com/user-attachments/assets/3adfa89e-55e1-41ba-8ca5-8030278e4a8a)

## mypage
### 내 회원정보 수정 가능(가입일, 아이디 read only 수정불가). 자신이 판매중인 책 확인 가능(책 제목, 등록일, 삭제). 자신이 게시판에 작성한 글 확인 가능
![Image](https://github.com/user-attachments/assets/a133e355-875f-443e-a49a-f8f189793741)

## bookregister
### 자신이 판매 할 책의 정보를 입력 후 등록
![Image](https://github.com/user-attachments/assets/bfe4bb49-9721-4537-96d4-3a9105216ec6)
